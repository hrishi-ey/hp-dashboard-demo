...
function GoogleMapComponent({ stores }) {
  ...
  const { isLoaded } = useJsApiLoader({
    ...
    googleMapsApiKey: "AIzaSyATdBkKcsZR_L2oirMeSMSxRojyLH2nAvE"
  });
  ...
  const onLoad = useCallback(function callback(map) {
    ...
    const bounds = new window.google.maps.LatLngBounds(stores[0].position);
  ...
  return (
    <div ref={wrapperRef} className='flex-grow w-full h-full relative transition-all' id='identify-me'>
      {
        isLoaded && wrapperSize && wrapperSize.width?
        <GoogleMap
          mapContainerStyle={{width: wrapperSize.width, height: wrapperSize.height}}
          onLoad={onLoad}
          onUnmount={onUnmount}
          zoom={2}
        ...
        >
          {
            stores.map((store) => {
              const customMarker = {
                ...
                fillColor: getColor(store.uptime),
                fillOpacity: 2,
                rotation: 0,
                scale: 1,
