...
export const loginUser = createAsyncThunk(
  ...
  async(creds) => {
        ...
        for (let index = 0; index < inv.data.length; index++) {
          ...
          if(element.type === "KPI_measurement" && element.name === "HennyPenny") {
            if(inventory.userType === null) {
              ...
              inventory.uptimeScore = element.adminKPI["Admin.Uptime.Score"].value;
              inventory.uptimeIndicator = element.adminKPI["Admin.Uptime.Indicator"].value;
              inventory.operationScore = element.adminKPI["Admin.Operation.Score"].value;
              inventory.operationIndicator = element.adminKPI["Admin.Operation.Indicator"].value;
              inventory.efficiencyScore = element.adminKPI["Admin.Efficiency.Score"].value;
              inventory.efficiencyIndicator = element.adminKPI["Admin.Efficiency.Indicator"].value;
    ...
    return {user: user.data, id:user.data.id, role:null, isLoggedIn: true, inventory};
  }
);
